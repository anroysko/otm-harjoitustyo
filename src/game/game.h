...
struct BlockType {
	...
	bool destroyable;
	/// How hard blocks can this block crush?
	int crush_strength;
	/// How hard is this block to crush
	int hardness;
	/// Base texture for this block.
	int base_texture;

	/// Constructs a block type with the given parameters.
	BlockType(bool can_fall, bool collectible, bool round, bool destroyable, int hardness, int crush_strength, int base_texture);
};
...
/// Contains the current game state, and has functions for updating it.
struct Level {
private:
	/// Name of the level.
	std::string name;
	...
	/// Can this block be pushed in the given direction?
	bool canPush(int place, bool dir);
...
public:
	...
	DrawData update(int move, int default_cam_x, int default_cam_y);
